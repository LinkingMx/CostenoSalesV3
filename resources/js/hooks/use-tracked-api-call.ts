...
export function useTrackedApiCall(options: UseTrackedApiCallOptions) {
    const { registerApiCall, completeApiCall, getFailedCallsInfo, retryFailedCalls } = useApiLoadingContext();

    const { componentName, defaultMetadata = {}, autoRetry = true, maxRetries = 3 } = options;

    const makeTrackedCall = React.useCallback(
        async <T>(
        ...
        [componentName, defaultMetadata, autoRetry, maxRetries, registerApiCall, completeApiCall],
    );
    ...
    const batchTrackedCalls = React.useCallback(
        async <T>(
            apiCalls: Array<{
                id: string;
                apiFunction: () => Promise<T>;
            ...
            const { batchId = `batch-${componentName}-${Date.now()}`, sequential = false, continueOnError = true } = options || {};
