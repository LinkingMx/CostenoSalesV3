...
export const useMonthlyChart = (
  ...
  options: UseMonthlyChartOptions = {}
): UseMonthlyChartReturn => {
  const {
    enableRetry = true,
    maxRetries = 3,
    debounceMs = 300,
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`🔄 useMonthlyChart useEffect triggered - effectId: ${effectId}, isValidForMonthlyChart: ${isValidForMonthlyChart}, selectedDateRange:`, selectedDateRange);
    }
