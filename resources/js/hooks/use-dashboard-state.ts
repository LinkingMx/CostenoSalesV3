...
export function useDashboardState() {
      ...
      if (stored) {
        ...
        if (parsed.currentDateRange?.from) {
          ...
          if (storedDate.getTime() !== today.getTime()) {
            console.log('🧹 Clearing old dashboard state - stored date was:', storedDate.toDateString(), 'but today is:', today.toDateString());
   ...
   */
  const setOriginalDateRange = useCallback((dateRange: DateRange | undefined) => {
   ...
   */
  const setCurrentDateRange = useCallback((dateRange: DateRange | undefined) => {
    const validatedRange = validateDateRange(dateRange);
    updateState({
      currentDateRange: validatedRange
    });
   ...
   */
  const getCachedApiResponse = useCallback((componentName: string, dateRange: DateRange | undefined) => {
   ...
   */
  const clearApiCache = useCallback((pattern?: string) => {
    if (pattern) {
      const newResponses = new Map(state.apiResponses);
   ...
   */
  const isStateValid = useCallback((maxAge: number = DEFAULT_CACHE_TTL): boolean => {
    const now = Date.now();
