...
import { BranchApiData, MainDashboardApiResponse, MainDashboardRequest } from './types';
 ...
 */
const processBranchData = (branchName: string, branchData: BranchApiData, previousBranchData?: BranchApiData): BranchSalesData => {
 ...
 */
const processApiResponse = (currentResponse: MainDashboardApiResponse, previousResponse?: MainDashboardApiResponse): BranchSalesData[] => {
    ...
    const previousCards = previousResponse?.success && previousResponse.data?.cards ? previousResponse.data.cards : {};
...
export const fetchMainDashboardData = async (
    ...
    const requestPromise = (async () => {
        try {
            ...
            const currentResponse = await apiPost<MainDashboardApiResponse>(API_ENDPOINTS.MAIN_DASHBOARD, currentRequest);
            ...
            if (includePreviousWeek && startDate === endDate) {
                try {
                    ...
                    previousResponse = await apiPost<MainDashboardApiResponse>(API_ENDPOINTS.MAIN_DASHBOARD, previousRequest);
                } catch (previousError) {
