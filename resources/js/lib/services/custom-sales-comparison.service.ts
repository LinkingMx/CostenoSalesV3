...
import { formatDateForApi, dateFromApiString } from './main-dashboard.service';
...
const processSingleDateData = (
  date: Date,
  apiResponse: MainDashboardApiResponse | null,
  isInRange: boolean = true
): SalesCustomData => {
  const totalSales = apiResponse?.success && apiResponse.data?.sales?.total
    ? apiResponse.data.sales.total
    : 0;
...
const fetchSingleDateData = async (date: Date): Promise<MainDashboardApiResponse | null> => {
  try {
    ...
    const response = await apiPost<MainDashboardApiResponse>(
