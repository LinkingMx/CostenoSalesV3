 ...
 */
function buildStoreDetailsRequest(storeId: number, dateRange: DateRange): StoreDetailsRequest {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                const totalSales = (result.data?.food?.money || 0) + (result.data?.drinks?.money || 0) + (result.data?.others?.money || 0);
