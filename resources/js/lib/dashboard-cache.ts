...
export const isStateValid = (
  state: CachedDashboardState | null,
...
class DashboardCacheManager {
        ...
        inertiaApp.on('navigate', (event: any) => {
          ...
          if (component === 'dashboard') {
            ...
            const isReturningFromBranch =
              this.metadata.navigationSource === 'dashboard' &&
              timeSinceLastNav < this.NAVIGATION_GRACE_PERIOD;
  ...
  private isValidEntry(entry: CacheEntry, dateRange: DateRange | undefined): boolean {
    ...
    return Math.abs(entryFrom - requestFrom) < tolerance &&
           Math.abs(entryTo - requestTo) < tolerance;
  }
...
export const getDashboardCache = <T>(
  componentName: string,
  dateRange: DateRange | undefined
): T | null => {
...
export const setDashboardCache = <T>(
  componentName: string,
  dateRange: DateRange | undefined,
  data: T,
  ttl?: number
