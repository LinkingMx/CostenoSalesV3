...
export function validateSalesDayData(salesData: SalesDayData[]): ValidationResult {
    ...
    const dateStrings = salesData.filter((item) => item.date instanceof Date).map((item) => item.date.toDateString());
 ...
 */
export function convertProcessedChartDataToSalesData(chartData: ProcessedChartData, selectedDate: Date): SalesDayData[] {
