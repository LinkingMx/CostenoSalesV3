/**
 * Conditional logging utility for custom-sales-comparison component
 *
 * Features:
 ...
 */
const DEBUG_CUSTOM_SALES = (() => {
    if (typeof window === 'undefined' || process.env.NODE_ENV !== 'development') {
        return false;
    }
    return window.localStorage.getItem('debug:custom-sales') === 'true';
})();
...
export const logger = {
    ...
    debug: (message: string, data?: unknown): void => {
        ...
        console.debug(`[CustomSales] ${message}`, logData);
    },
    ...
    warn: (message: string, data?: unknown): void => {
        ...
        console.warn(`[CustomSales] ${message}`, data);
    },
    ...
    error: (message: string, data?: unknown): void => {
        console.error(`[CustomSales] ${message}`, data);
    },
    ...
    info: (message: string, data?: unknown): void => {
        if (!DEBUG_CUSTOM_SALES) return;
        console.info(`[CustomSales] ${message}`, data);
    },
};

/**
 * Enable debug logging in browser console:
 * localStorage.setItem('debug:custom-sales', 'true')
 *
 * Disable debug logging:
 * localStorage.removeItem('debug:custom-sales')
 */