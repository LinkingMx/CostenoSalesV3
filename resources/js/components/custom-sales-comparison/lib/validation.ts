...
import type { CustomRangeMetrics, SalesCustomData, ValidationResult } from '../types';
    ...
    if (!dateRange) {
        ...
        if (dateRange.from && dateRange.to && dateRange.from instanceof Date && dateRange.to instanceof Date) {
            if (!isCustomRangeSelected(dateRange)) {
                errors.push(
                    'Date range must represent a custom range (not single day, complete week, or complete month) for custom sales comparison',
                );
            }
 ...
 */
export function calculateCustomRangeMetrics(salesData: SalesCustomData[], dateRange: DateRange): CustomRangeMetrics {
    if (!salesData || salesData.length === 0) {
        return {
            totalSales: 0,
            averageDaily: 0,
