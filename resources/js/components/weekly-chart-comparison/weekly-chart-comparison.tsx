 ...
 * ```
 */
export const WeeklyChartComparison = React.memo(function WeeklyChartComparison({ selectedDateRange }: WeeklyChartComparisonProps) {
    ...
    const isValidCompleteWeek = React.useMemo(() => isCompleteWeekSelected(selectedDateRange), [selectedDateRange]);
    ...
    if (error) {
        return <WeeklyChartError message={error} onRetry={refetch} isRetrying={isLoading} />;
    }
    ...
    return (
        <div className="w-full rounded-xl border border-border bg-card p-6">
            ...
            <div
                ...
                style={{ outline: 'none' }}
            >
                <MemoizedWeeklyComparisonChart data={data} height={225} showLegend={true} showGrid={true} />
            </div>
        </div>
    );
});
