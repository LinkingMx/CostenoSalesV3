...
interface MonthlyErrorBoundaryProps {
    children: ReactNode;
    fallback?: ReactNode;
...
/**
 * MonthlyErrorBoundary - Error boundary specific to monthly sales comparison component.
 *
 * Provides graceful error handling and recovery for the monthly sales component.
 * Displays user-friendly error messages and allows recovery without full page reload.
 *
 * @component
 * @param {MonthlyErrorBoundaryProps} props - Error boundary configuration
 * @returns {JSX.Element} Protected component wrapper or error fallback UI
 *
 * @description Features:
 * - Catches JavaScript errors in monthly sales component tree
 ...
 * @example
 * ```tsx
 * <MonthlyErrorBoundary onError={handleError}>
 *   <MonthlySalesComparison selectedDateRange={range} />
 * </MonthlyErrorBoundary>
 * ```
 */
export class MonthlyErrorBoundary extends React.Component<MonthlyErrorBoundaryProps, ErrorBoundaryState> {
    constructor(props: MonthlyErrorBoundaryProps) {
        super(props);
        this.state = {
            hasError: false,
            error: null,
    ...
    componentDidCatch(error: Error, errorInfo: ErrorInfo) {
        ...
        if (process.env.NODE_ENV === 'development') {
            console.group('🚨 Monthly Sales Component Error');
    ...
    render() {
        ...
        if (this.state.hasError) {
            ...
            // Default error UI for monthly sales component
            return (
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 space-y-3">
                                <div>
                                    <h3 className="font-semibold text-destructive">Error en Ventas Mensuales</h3>
                                    <p className="mt-1 text-sm text-muted-foreground">
                                        No se pudieron cargar los datos de ventas mensuales. Esto puede ser temporal.
                                    </p>
                                </div>
                                ...
                                {process.env.NODE_ENV === 'development' && this.state.error && (
                                    <details className="mt-3">
                                        ...
                                        <div className="mt-2 rounded-md bg-muted p-3">
                                            <p className="font-mono text-xs break-words text-muted-foreground">{this.state.error.message}</p>
                                            {this.state.error.stack && (
                                                <pre className="mt-2 overflow-auto text-xs text-muted-foreground">{this.state.error.stack}</pre>
                                            )}
                                        </div>
                                    </details>
                                )}
...
export default MonthlyErrorBoundary;
