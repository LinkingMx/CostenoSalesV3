/**
 * Conditional logging utility for weekly-sales-comparison component
 *
 * Features:
 ...
 */
const DEBUG_WEEKLY_SALES = (() => {
    if (typeof window === 'undefined' || process.env.NODE_ENV !== 'development') {
        return false;
    }
    return window.localStorage.getItem('debug:weekly-sales') === 'true';
})();
...
export const logger = {
    ...
    debug: (message: string, data?: unknown): void => {
        ...
        console.debug(`[WeeklySales] ${message}`, logData);
    },
    ...
    warn: (message: string, data?: unknown): void => {
        ...
        console.warn(`[WeeklySales] ${message}`, data);
    },
    ...
    error: (message: string, data?: unknown): void => {
        console.error(`[WeeklySales] ${message}`, data);
    },
    ...
    info: (message: string, data?: unknown): void => {
        if (!DEBUG_WEEKLY_SALES) return;
        console.info(`[WeeklySales] ${message}`, data);
    },
};

/**
 * Enable debug logging in browser console:
 * localStorage.setItem('debug:weekly-sales', 'true')
 *
 * Disable debug logging:
 * localStorage.removeItem('debug:weekly-sales')
 */