...
import { Loader2, AlertCircle, RefreshCcw, Clock } from 'lucide-react';
...
export function FullPageLoadingOverlay({
  ...
  const stage = React.useMemo(() => getLoadingStage(progress, hasErrors, activeCallsCount), [
    progress,
    hasErrors,
    activeCallsCount
  ]);
        ...
        <div className="text-center space-y-4 w-full">
          ...
          {showDetails && process.env.NODE_ENV === 'development' && (
            <details className="text-left">
              ...
              <div className="mt-2 space-y-1 text-xs text-muted-foreground bg-muted/50 p-3 rounded-md">
        ...
        {hasErrors && failedCallsInfo.length > 0 && (
          <Alert className="max-w-sm">
            ...
            <AlertDescription className="flex flex-col space-y-2">
              <span>
                {failedCallsInfo.length === 1
                  ? 'Una solicitud falló'
                  : `${failedCallsInfo.length} solicitudes fallaron`
                }. Los datos parciales se mostrarán correctamente.
              </span>
