/**
 * Conditional logging utility for custom-sales-branches component
 *
 * Features:
 ...
 */
const DEBUG_CUSTOM_BRANCHES = (() => {
    if (typeof window === 'undefined' || process.env.NODE_ENV !== 'development') {
        return false;
    }
    return window.localStorage.getItem('debug:custom-branches') === 'true';
})();
...
export const logger = {
    ...
    debug: (message: string, data?: unknown): void => {
        if (!DEBUG_CUSTOM_BRANCHES) return;
        ...
        console.debug(`[CustomBranches] ${message}`, logData);
    },
    ...
    warn: (message: string, data?: unknown): void => {
        ...
        console.warn(`[CustomBranches] ${message}`, data);
    },
    ...
    error: (message: string, data?: unknown): void => {
        console.error(`[CustomBranches] ${message}`, data);
    },
    ...
    info: (message: string, data?: unknown): void => {
        if (!DEBUG_CUSTOM_BRANCHES) return;
        console.info(`[CustomBranches] ${message}`, data);
    },
};

/**
 * Enable debug logging in browser console:
 * localStorage.setItem('debug:custom-branches', 'true')
 *
 * Disable debug logging:
 * localStorage.removeItem('debug:custom-branches')
 */