    ...
    if (branch.averageTicket !== undefined && branch.totalSales !== undefined && branch.totalTickets !== undefined) {
      if (branch.totalTickets > 0) {
        ...
        if (Math.abs(branch.averageTicket - calculatedAverage) > tolerance) {
          warnings.push(`${branchLabel}: averageTicket (${branch.averageTicket}) doesn't match totalSales / totalTickets (${calculatedAverage.toFixed(2)})`);
        }
      }
    }
 ...
 */
export function calculateCustomBranchMetrics(
  branchData: BranchCustomSalesData[], 
  dateRange: DateRange
): CustomBranchMetrics {
  if (!branchData || branchData.length === 0) {
    return {
