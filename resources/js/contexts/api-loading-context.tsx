...
export const ApiLoadingProvider: React.FC<ApiLoadingProviderProps> = ({ children }) => {
    ...
    const completedCallsCount = React.useMemo(() => {
        return Array.from(activeApiCalls.values()).filter((call) => call.status === 'completed' || call.status === 'failed').length;
    }, [activeApiCalls]);
    ...
    const registerApiCall = React.useCallback(
        (id: string, componentName: string, metadata?: ApiCallMetadata) => {
    ...
    const onDateRangeChange = React.useCallback(
        (range: DateRange | undefined) => {
            ...
            const hasDateChanged = (() => {
                ...
                return Math.abs(fromTime1! - fromTime2!) > tolerance || Math.abs(toTime1! - toTime2!) > tolerance;
            })();
    ...
    return <ApiLoadingContext.Provider value={contextValue}>{children}</ApiLoadingContext.Provider>;
};
