...
- **File Creation Policy**: NEVER create files unless absolutely necessary. Always prefer editing existing files over creating new ones.
- **Documentation**: NEVER proactively create documentation files (*.md) or README files unless explicitly requested.
- **Code Quality**: Always run linting and type checking commands before completing tasks.

### Required Commands Before Completion

When completing development tasks, ALWAYS run these commands to ensure code quality:
- `vendor/bin/pint` - Fix PHP code style
- `npm run lint` - Lint and fix JavaScript/TypeScript
- `npm run types` - Run TypeScript type checking
...
- **Service Worker**: Configured via Vite PWA plugin with Workbox
- **Manifest**: Available at `/manifest.json` for app installation
- **Theme Colors**: Responsive theme-color meta tags for light/dark mode
- **Apple Touch Icons**: Configured for iOS devices
- **Status Bar**: Black-translucent style for mobile web apps

## API Architecture

### Backend API Structure
- **Session-based Authentication**: Uses Laravel's built-in session authentication
- **Inertia Middleware**: `HandleInertiaRequests` processes all page requests
- **Appearance Middleware**: `HandleAppearance` manages theme preferences
- **API Endpoints**: Located in `app/Http/Controllers/Api/` for JSON responses
- **Form Requests**: Validation handled via dedicated Request classes

### Frontend Integration
- **Type-safe Routing**: Generated by Wayfinder plugin in `resources/js/actions/`
- **Form Handling**: Inertia Form components with automatic CSRF protection
- **Error Handling**: Centralized error display via `InputError` component

## Mobile Development

### Responsive Design
- **Mobile-first Approach**: TailwindCSS utilities with mobile-first breakpoints
- **Touch Optimization**: Components designed for touch interfaces
- **Viewport Configuration**: Optimized meta viewport for mobile browsers
- **PWA Features**: Installation prompts and offline capability

### Device Detection
- **Mobile Hook**: Custom `useMobile` hook available for responsive logic
- **Theme Detection**: Automatic system theme detection with manual override
- **Safe Areas**: Proper handling of notches and safe areas on modern devices

## Single Test Execution

To run individual tests, use these patterns:
```bash
